...
namespace CPPMathLibrary {
	...
	namespace SimultaneousDiophantine {
		...
		__declspec(dllexport) QSMatrix<double> SameDivisor(const std::vector<double>& x, const double& alpha, const double& epsilon) throw (IncorrectDimensionException*);
		...
		__declspec(dllexport) QSMatrix<double> SameDivisorFromRealVector(const std::vector<double>& x, const double& alpha, const double& epsilon, const size_t& quotientCount) throw (IncorrectDimensionException*);
